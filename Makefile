...
CC=clang
LIB_EXT=dylib
CFLAGS=-x objective-c -DSOKOL_METAL -fenable-matrix -framework Metal -framework Cocoa -framework IOKit -framework MetalKit -framework Quartz -framework AudioToolbox
ARCH:=$(shell uname -m)
ifeq ($(ARCH),arm64)
	ARCH=osx_arm64
else
