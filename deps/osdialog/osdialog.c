...
char* osdialog_strndup(const char* s, size_t n) {
	if (!s)
		return NULL;
	char* d = (char*)OSDIALOG_MALLOC(n + 1);
	memcpy(d, s, n);
	d[n] = '\0';
	return d;
}
...
osdialog_filters* osdialog_filters_parse(const char* str) {
	...
	osdialog_filters* filters_head = (osdialog_filters*)OSDIALOG_MALLOC(sizeof(osdialog_filters));
	...
	while (1) {
		...
		if (!patterns) {
			...
			if (c == ':') {
				...
				filters->patterns = (osdialog_filter_patterns*)OSDIALOG_MALLOC(sizeof(osdialog_filter_patterns));
			...
			if (c == ',') {
				...
				patterns->next = (osdialog_filter_patterns*)OSDIALOG_MALLOC(sizeof(osdialog_filter_patterns));
				...
				patterns->pattern = osdialog_strndup(text, str - text);
				patterns = NULL;
				filters->next = (osdialog_filters*)OSDIALOG_MALLOC(sizeof(osdialog_filters));
...
static osdialog_filter_patterns* osdialog_filter_patterns_copy(const osdialog_filter_patterns* src) {
	...
	osdialog_filter_patterns* dest = (osdialog_filter_patterns*)OSDIALOG_MALLOC(sizeof(osdialog_filter_patterns));
...
osdialog_filters* osdialog_filters_copy(const osdialog_filters* src) {
	...
	osdialog_filters* dest = (osdialog_filters*)OSDIALOG_MALLOC(sizeof(osdialog_filters));
